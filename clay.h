...
#define CLAY__SIZING_FIT_INTERNAL(...) \
    CLAY__INIT(Clay_SizingAxis) { .sizeMinMax = CLAY__INIT(Clay_SizingMinMax) __VA_ARGS__, .type = CLAY__SIZING_TYPE_FIT }
...
#define CLAY__SIZING_GROW_INTERNAL(...) \
    CLAY__INIT(Clay_SizingAxis) { .sizeMinMax = CLAY__INIT(Clay_SizingMinMax) __VA_ARGS__, .type = CLAY__SIZING_TYPE_GROW }
...
#define CLAY_SIZING_FIXED(fixedSize) \
    CLAY__INIT(Clay_SizingAxis) { .sizeMinMax = {fixedSize, fixedSize}, .type = CLAY__SIZING_TYPE_FIXED }

#define CLAY_SIZING_PERCENT(percentOfParent) \
    CLAY__INIT(Clay_SizingAxis) { .sizePercent = (percentOfParent), .type = CLAY__SIZING_TYPE_PERCENT }
...
#define CLAY_STRING(string) \
    CLAY__INIT(Clay_String) { .length = CLAY__STRING_LENGTH(string), .chars = (string) }
...
Clay_ScrollElementConfig CLAY__SCROLL_ELEMENT_CONFIG_DEFAULT = CLAY__INIT(Clay_ScrollElementConfig){};
...
Clay_BorderElementConfig CLAY__BORDER_ELEMENT_CONFIG_DEFAULT = CLAY__INIT(Clay_BorderElementConfig){};
...
Clay_LayoutElementHashMapItem *Clay__AddHashMapItem(Clay_ElementId elementId, Clay_LayoutElement *layoutElement)
{
    ...
    { // Just replace collision, not a big deal - leave it up to the end user
        ...
        { // Collision - resolve based on generation
            ...
            if (hashItem->generation <= Clay__generation)
            { // First collision - assume this is the "same" element
...
void Clay__CloseElement()
{
    ...
    {
        if (layoutConfig->sizing.width.sizeMinMax.max <= 0)
    ...
    {
        if (layoutConfig->sizing.height.sizeMinMax.max <= 0)
        { // Set the max size if the user didn't specify, makes calculations easier
...
void Clay__CalculateFinalLayout()
{
    ...
    for (int rootIndex = 0; rootIndex < Clay__layoutElementTreeRoots.length; ++rootIndex)
    {
        ...
        if (Clay__ElementHasConfig(rootElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING_CONTAINER) && parentHashMapItem)
        {
            ...
            switch (config->attachment.parent)
            { // I know I could merge the x and y switch statements, but this is easier to read
            case CLAY_ATTACH_POINT_LEFT_TOP:
            case CLAY_ATTACH_POINT_RIGHT_TOP:
            case CLAY_ATTACH_POINT_CENTER_TOP:
...
Clay__DebugElementConfigTypeLabelConfig Clay__DebugGetElementConfigTypeLabel(Clay__ElementConfigType type)
{
    ...
    {
    case CLAY__ELEMENT_CONFIG_TYPE_RECTANGLE:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Rectangle"), CLAY__INIT(Clay_Color){243, 134, 48, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_TEXT:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Text"), CLAY__INIT(Clay_Color){105, 210, 231, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_IMAGE:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Image"), CLAY__INIT(Clay_Color){121, 189, 154, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_FLOATING_CONTAINER:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Floating"), CLAY__INIT(Clay_Color){250, 105, 0, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_SCROLL_CONTAINER:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Scroll"), CLAY__INIT(Clay_Color){242, 196, 90, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_BORDER_CONTAINER:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Border"), CLAY__INIT(Clay_Color){108, 91, 123, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_CUSTOM:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Custom"), CLAY__INIT(Clay_Color){11, 72, 107, 255}};
    }
...
void Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta, float deltaTime)
{
    ...
    for (int i = 0; i < Clay__scrollContainerDatas.length; i++)
    {
        ...
        for (int j = 0; j < Clay__pointerOverIds.length; ++j)
        { // TODO n & m are small here but this being n*m gives me the creeps
...
{ // TODO return priority for separating multiple results
    for (int i = 0; i < Clay__pointerOverIds.length; ++i)
    {
